...
const searchQuery = (query: string, days: string) => `
      {
    "upcomingTrips": *[_type == "upcomingTripDetail" && (title match "${query}" || desc match "${query}" || destination match "${query}") ${days ? `&& (durationn == "${days}" || durationd == "${days}")` : ""}] {
      title,
      cover,
      durationn,
      durationd,
      ...
      "relatedTrips": *[_type == "upcomingTripDetail" ${days ? `&& !(durationn == "${days}" || durationd == "${days}")` : ""} && (title match "${query}" || desc match "${query}" || destination match "${query}")] {
      title,
      cover,
      durationn,
      durationd,
      ...
      {upcomingTrips.length === 0 && (
        <div className="flex flex-col py-6  gap-4 md:gap-9">
          <h3 className="text-secondary-oncontainer titles md:titlem lg:titlel">
            No trips available for {days} days. Check our related packages below.
          </h3>
        </div>
      )}
